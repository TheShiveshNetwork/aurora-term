...
4. shell — last resort. Reserve it for: build commands, test runners, git operations,
   ...
   when a dedicated tool covers it — reading a file means read_file, never
   cat/Get-Content/type. If a shell command fails because no terminal session is available
   in this context, stop using shell entirely and switch to the dedicated file tools.
