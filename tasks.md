...
> In OpenCode, the limit and offset parameters are used by the AI agent to read large files in smaller, manageable chunks. This prevents the agent from dumping an entire file at once and overflowing the AI model's context window
...
MAC OS Tasks
1. header logo has margin left, remove it.
2. bash not loading on terminal
