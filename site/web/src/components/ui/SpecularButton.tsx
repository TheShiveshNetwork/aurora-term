...
export default function SpecularButton({
  ...
  useEffect(() => {
    ...
    const onPointerMove = (e: PointerEvent) => {
      ...
      const t = Math.max(0, 1 - dist / Math.max(propsRef.current.proximity ?? 1, 1));
    ...
    const update = (now: number) => {
      ...
      program.uniforms.uRadius.value = Math.min(p.radius ?? 16, Math.min(sizeRef.w, sizeRef.h) / 2) * dpr;
